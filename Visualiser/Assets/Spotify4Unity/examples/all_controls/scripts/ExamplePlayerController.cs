...
public class ExamplePlayerController : SpotifyUIBase
{
    ...
    [SerializeField, Tooltip("Information about the current song an album art, gets enabled once connected.")]
    private GameObject m_informationCanvas = null;

    [SerializeField, Tooltip("Media controls for Spotify, gets enabled once connected")]
    private GameObject m_mediaButtonsCanvas = null;

    [SerializeField, Tooltip("Slider Representing the time of the current song and a text field, gets enabled once connected.")]
    private GameObject m_playingStatusCanvas = null;

    [SerializeField, Tooltip("Volume Buttons and slider, gets enabled once connected")]
    private GameObject m_volumeCanvas = null;

    [SerializeField, Tooltip("Sign Out button, gets enabled once connected")]
    private GameObject m_signOutCanvas = null;
    ...
    private void Start()
    {
        ...
        m_informationCanvas.SetActive(false);
        m_mediaButtonsCanvas.SetActive(false);
        m_playingStatusCanvas.SetActive(false);
        m_volumeCanvas.SetActive(false);
        m_signOutCanvas.SetActive(false);
    ...
    protected override void OnConnectingChanged(ConnectingChanged e)
    {
        ...
        // Enable Information Canvas when Not connecting and connected
        m_informationCanvas.SetActive(!e.IsConnecting && SpotifyService.IsConnected);
        // Enable Media Buttons Canvas when Not connecting and connected
        m_mediaButtonsCanvas.SetActive(!e.IsConnecting && SpotifyService.IsConnected);
        // Enable Playing Status Canvas when Not connecting and connected
        m_playingStatusCanvas.SetActive(!e.IsConnecting && SpotifyService.IsConnected);
        // Enable Volume Canvas when Not connecting and connected
        m_volumeCanvas.SetActive(!e.IsConnecting && SpotifyService.IsConnected);
        // Enable Sign Out Canvas when Not connecting and connected
        m_signOutCanvas.SetActive(!e.IsConnecting && SpotifyService.IsConnected);
    }
